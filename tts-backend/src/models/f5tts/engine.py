...
Provides:
- F5TTSEngine implementing TTSEngine interface
...
from src.models.f5tts.inference import (
    F5TTSInference,
    MockF5TTSInference,
    get_inference,
)
from src.models.f5tts.voice_manager import VoiceManager
...
class F5TTSEngine(TTSEngine):
    """
    F5-TTS Engine implementing TTSEngine interface.

    Uses F5-TTS for high-quality voice cloning with natural speech.
    """
    ...
    def __init__(self, config: F5TTSConfig | None = None) -> None:
        ...
        self._inference: F5TTSInference | MockF5TTSInference | None = None
    ...
    async def load(self) -> None:
        ...
        logger.info(f"Loading F5-TTS engine (mock={self._config.use_mock})...")
        ...
        self._voice_manager = VoiceManager(
            voices_directory=self._config.voices_directory,
    ...
    async def unload(self) -> None:
        ...
        logger.info("Unloading F5-TTS engine...")
    ...
    async def synthesize(
        ...
        # Get prompt audio path and reference text
        prompt_audio_path = await self._voice_manager.get_reference_audio_path(options.voice_id)
        prompt_text = await self._voice_manager.get_reference_text(options.voice_id)

        # Convert Path to string if exists
        prompt_audio_str = str(prompt_audio_path) if prompt_audio_path else None

        if not prompt_audio_str:
            raise RuntimeError(
                f"No reference audio found for voice '{options.voice_id}'. "
        ...
        # Get speaker embedding (for compatibility)
    ...
    async def clone_voice(
        ...
        # Use first sample for voice cloning
        if not samples:
        ...
        metadata = {
            ...
            "reference_text": "",  # User can update this later
        }
        ...
        # Register speaker (F5-TTS doesn't need explicit registration)
        try:
